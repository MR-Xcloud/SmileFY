...
class imageapi(APIView):
    def post(self, request):
        ...
        # Convert file1 to OpenCV array
        file1_array = cv2.imread(file1_save_path)
        if file1_array is None:
            return Response({"error": "Error reading file1: File is empty or not loaded."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not file2:
            ...
            try:
                file2_array = cv2.imdecode(np.frombuffer(file2.read(), np.uint8), cv2.IMREAD_COLOR)
                if file2_array is None:
                    return Response({"error": "Error decoding file2: Could not read image."}, status=status.HTTP_400_BAD_REQUEST)
            except Exception as e:
                return Response({"error": f"Error reading file2: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
